...
class _ListeUserState extends State<ListeUser> {
  ...
  Future<List<User>> _getUsers() async {
    ...
    String url = Pub.chemin + "get_user.php";
    var data = await http.get(url);
    var jsonData = json.decode(data.body);

    List<User> users = [];
    for (var u in jsonData) {
      User user = User(int.parse(u["CodeUser"]), u["Username"], u["Password"]);
      users.add(user);
    }
    setState(() {
      _isLoading = false;
  ...
  Widget build(BuildContext context) {
    return new Scaffold(
      appBar: new AppBar(
        ...
        title: new Text("LISTE D'UTILISATEUR"),
      ),
      drawer: DrawScreen(),
      body: Container(
          child: myData != null
              ? ListView.builder(
                  ...
                  itemBuilder: (BuildContext context, int index) {
                    return ListTile(
                      ...
                      subtitle: Text("Mot de passe: " + myData[index].password),
